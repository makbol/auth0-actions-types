...
export interface ApiRedirect {
   ...
   * @param options.expiresInSeconds Number of seconds before this token will expire (defaults to 900).
   * @param options.payload The data intended to be passed to the target of the redirect and whose authenticity and integrity must be provable.
   * @param options.secret A secret that will be used to sign a JWT that is shared with the redirect target. The secret value should be stored as a secret and retrieved using event.secrets['SECRET_NAME'].
   ...
   * @param options.query Additional query string parameters that should be appended to the redirect URL.
   ...
   * @param options.secret Secret used to encode the token.
   * @param options.tokenParameterName The name of the query or body parameter that was sent to the /continue endpoint. (defaults to session_token)
   * @returns Returns payload of the JWT token.
   */
  validateToken<T>(options: { secret: string; tokenParameterName: string }): T
}
